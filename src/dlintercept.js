        ...
        !in_skip_list)
    {
        ...
        if (file)
        {
            ...
            else
            {
                ...
                downloadInfo.httpCookies = rd.cookies;
    ...
    downloadInfo, details, callbackFn)
{
    ...
    {
        this.passDownloadToNadeko(downloadInfo, details, callbackFn);
    }
    else
    {
...
DownloadsInterceptManager.prototype.passDownloadToNadeko = function(
    downloadInfo, details, callbackFn)
{
    ...
    const endpoint = `http://localhost:${nadekoServerPort}`;

    try {
        console.debug(`[Background Script] Sending URL to Nadeko App: ${url} (Filename: ${filename}) on port ${nadekoServerPort}`);
