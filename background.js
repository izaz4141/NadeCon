...
class WebSocketClient {
    ...
    connect() {
        ...
        const url = `ws://${nadekoBindAddress}:${nadekoServerPort}/ws?key=${encodeURIComponent(nadekoApiKey)}`;
...
async function initConfig() {
    try {
        ...
        console.debug(`[Background Script] Config initialized - showPopup: ${showPopup} (raw value: ${result.showPopup}, type: ${typeof result.showPopup})`);
...
async function sendUrlToApp(url, filename = null) {
    ...
    console.debug(`[Background Script] Sending URL to Nadeko App: ${url} (Filename: ${filename})`);
...
function deriveFilename(url, contentType, contentDisposition) {
    ...
    if (contentType) {
        ...
        if (extensions) {
            ...
            if (typeParts.length > 1) {
                ...
                if (fileExtension === 'octet-stream' && (url.includes('.bin') || url.includes('.dat'))) fileExtension = 'bin';
    ...
    if (fileExtension && currentExt.toLowerCase() !== fileExtension) {
        // Check if current extension is a known media extension
        const mediaExtensions = ['mp4', 'webm', 'ogg', 'mp3', 'wav', 'flac', 'aac', 'avi', 'mov', 'mkv', 'wmv', 'gif', 'jpg', 'jpeg', 'png', 'svg', 'webp', 'm3u8', 'mpd', 'ts', 'bin', 'dat'];
...
function parseContentDispositionFilename(contentDisposition) {
    ...
    let match = /filename\*\s*=\s*(?:UTF-8''|)([^;\s]+)/i.exec(contentDisposition);
    if (match && match[1]) {
        try {
            return decodeURIComponent(match[1].replace(/['"]/g, ''));
        } catch (e) {
            console.warn('[Download Intercept] Failed to decode RFC 5987 filename:', match[1]);
        }
    }
...
function isDownloadDisposition(contentDisposition) {
    ...
    return lower.includes('attachment') || /filename[*]?\s*=/i.test(contentDisposition);
}
...
async function getMediaDetails(url, details = null) {
    ...
    if (details) {
        ...
        ({ valid, contentType, contentDisposition, contentLength } = await fetchMediaHeaders(url));
    }
...
async function addMediaUrl(tabId, url, source, overrides = {}) {
    ...
    if (tabId >= 0 && showPopup && source !== 'contentScript') {
        ...
        browser.tabs.sendMessage(tabId, {
            ...
            console.debug(`[Background Script] Could not send showMediaPopup to tab ${tabId}: ${error.message}`);
        });
    } else {
        console.debug(`[Background Script] Skipping showMediaPopup for tabId: ${tabId} (Source: ${source}).`);
    }
}
...
function checkResponseHeaders(details) {
    ...
    if (!isKnown && ext) {
        ...
        const downloadExtensions = [
            ...
            'apk', 'ipa', 'iso', 'img',
            'mp4', 'mkv', 'webm', 'avi', 'mov', 'wmv', 'flv', 'mp3', 'wav', 'flac', 'aac', 'ogg', 'm4a', 'opus'
        ];
    ...
    if (contentType.startsWith('video/') || contentType.startsWith('audio/') || contentType.includes('mpegurl') || contentType.includes('dash+xml')) {
        return { action: 'download' };
    }
...
function checkRequest(details) {
    ...
    if (lowerUrl.includes('facebook.com') || lowerUrl.includes('fbcdn.net')) {
        if (lowerUrl.includes('/ajax/') || lowerUrl.includes('bootloader-endpoint') || lowerUrl.includes('graphql')) {
            return { action: 'ignore' };
        }
    }
    ...
    const knownMediaDomains = /(youtube\.com|googlevideo\.com|vimeo\.com|twitch\.tv|dailymotion\.com|soundcloud\.com|spotifycdn\.com|netflix\.com|disneyplus\.com|hbomax\.com)/i;
...
browser.webRequest.onBeforeRequest.addListener(
    (details) => {
        ...
        const relevantTypes = ['main_frame', 'sub_frame', 'other', 'xmlhttprequest', 'media', 'object'];
        if (relevantTypes.includes(details.type)) {
            ...
            if (check.action === 'monitor') {
                ...
                if (details.type === 'xmlhttprequest' || details.type === 'media' || details.type === 'object') {
                    addMediaUrl(details.tabId, details.url, 'webRequest').catch(e => { });
                }
            }
        }
    },
    { urls: ["<all_urls>"], types: ['main_frame', 'sub_frame', 'other', 'xmlhttprequest', 'media', 'object'] },
    ["requestBody"]
);
...
browser.webRequest.onBeforeRedirect.addListener(
    (details) => {
        ...
        if (request) {
            ...
            console.debug(`[Download Intercept] Redirect detected: ${request.url} -> ${details.redirectUrl}`);
        }
    },
    { urls: ["<all_urls>"], types: ['main_frame', 'sub_frame', 'other', 'xmlhttprequest', 'media', 'object'] }
);
...
browser.webRequest.onHeadersReceived.addListener(
    (details) => {
        ...
        if (check.action === 'download') {
            ...
            setTimeout(() => handleInterceptedDownload(
                finalUrl,
                contentType,
                contentDisposition,
                details.tabId,
 ...
 */
async function handleInterceptedDownload(url, contentType, contentDisposition, tabId, method = 'GET', requestBody = null, retryCount = 0) {
    console.debug(`[Download Intercept] Handling intercepted download: ${url}`);
    const isAlive = await isLocalhostAlive();
    const filename = deriveFilename(url, contentType, contentDisposition);

    if (isAlive) {
        console.debug(`[Download Intercept] Nadeko App is connected. Sending to Nadeko: ${url} as ${filename}`);
        ...
        try {
            ...
            if (method === 'POST' && requestBody) {
                if (requestBody.formData) {
                    ...
                    console.debug('[Download Intercept] POST request with raw data detected');
                }
            }
            ...
            if (retryCount < 2) {
                ...
                console.debug(`[Download Intercept] Retrying in ${retryDelay}ms... (attempt ${retryCount + 1}/2)`);

                setTimeout(() => {
                    handleInterceptedDownload(url, contentType, contentDisposition, tabId, method, requestBody, retryCount + 1);
                }, retryDelay);
                return;
            }

            // After retries exhausted, fall back to browser download
            console.error(`[Download Intercept] Failed after ${retryCount} retries. Falling back to browser download: ${url}`);
            fallbackToBrowserDownload(url, filename);
        }
    } else {
        console.debug(`[Download Intercept] Nadeko App is not connected. Using browser download for: ${url}`);
        fallbackToBrowserDownload(url, filename);
    }
}
...
function fallbackToBrowserDownload(url, filename) {
    browser.downloads.download({
        ...
        console.error(`[Download Intercept] Browser download also failed: ${url}`, error);
        ...
        browser.downloads.download({
            ...
            console.error(`[Download Intercept] All download methods failed: ${url}`, finalError);
        });
    });
}
...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.type === "getMediaUrls" && message.tabId !== undefined) {
        ...
        console.debug(`[Background Script] showPopup updated via configChanged: ${showPopup}`);
